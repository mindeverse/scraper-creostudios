"""Creo Studios Shopify parser — /products.json paginated (EN + EUR)."""
...
def _headers() -> dict[str, str]:
    return {
        ...
        # Force EUR presentment on Shopify storefront JSON (default store currency is GBP)
...
def _infer_gender(handle: str, category: Optional[str], tags: list[str]) -> Optional[str]:
    ...
    # Creo Studios is a unisex label; only split gender on explicit women/men
...
def fetch_all_products_json() -> list[dict[str, Any]]:
    """Paginate store-wide /products.json for full catalog coverage (~83 products)."""
...
def parse_shopify_product(
    ...
    # Creo Studios has no explicit back images in products.json, so back_image_url stays None
